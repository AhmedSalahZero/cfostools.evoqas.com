    ...
    <div class="min-h-screen bg-mp-page text-white">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-white text-sm mt-0.5">Overlay multiple data series on the same chart</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-mp-card border border-mp-border rounded-xl p-5 mb-6">
          <h2 class="text-white font-semibold mb-4 text-sm uppercase tracking-wider text-white">Select Series to Compare</h2>
          ...
          <p v-if="selectedSeriesIds.length === 0" class="text-white text-sm mt-3 italic">Select 2 or more series to compare them on the chart.</p>
          <p v-if="selectedSeriesIds.length >= 2 && hasDualAxis" class="text-white text-xs mt-3">
            ⚠️ Series have different units — dual Y-axis is shown. The left axis corresponds to the first selected series.
          </p>
        </div>

        <!-- CHART -->
        <div class="bg-mp-card border border-mp-border rounded-xl p-5 mb-6" v-if="selectedSeriesIds.length >= 1">
          ...
          <div style="height: 320px;" class="relative">
              ...
              <template v-if="compareTooltip.visible">
                ...
                <template v-for="pt in compareTooltip.points" :key="pt.id">
                  <circle v-if="pt.y !== null"
                    ...
                    :fill="pt.color" stroke="#112240" stroke-width="1.5"/>
                  <circle v-if="pt.y !== null"
            ...
            <Teleport to="body">
              <div v-if="compareTooltip.visible && compareSvgRef"
                ...
                <div class="bg-mp-card border border-mp-border rounded-xl shadow-2xl px-3.5 py-3 min-w-[160px]">
                  <p class="text-white text-xs font-mono mb-2 pb-2 border-b border-mp-border">
                  ...
                  <div v-for="pt in compareTooltip.points" :key="pt.id" class="flex items-center justify-between gap-4 mb-1 last:mb-0">
                    <div class="flex items-center gap-1.5">
                      ...
                      <span class="text-white text-xs truncate max-w-[90px]">{{ pt.name }}</span>
                    </div>
                    <span class="text-white font-semibold text-xs tabular-nums">
                      {{ formatValue(pt.value) }}
                      <span class="text-white font-normal">{{ pt.unit }}</span>
                    </span>
                  </div>
                </div>
              </div>
          ...
          <!-- Legend -->
          <div class="flex flex-wrap gap-x-6 gap-y-2 mt-4 pt-4 border-t border-mp-border">
            <div v-for="sd in chartSeriesData" :key="sd.id" class="flex items-center gap-2">
              ...
              <span class="text-xs text-white">{{ sd.name }}</span>
              <span class="text-xs text-white">({{ sd.unit }})</span>
            </div>
          </div>
        </div>

        <!-- STATS TABLE -->
        <div v-if="selectedSeriesIds.length >= 2" class="bg-mp-card border border-mp-border rounded-xl overflow-hidden">
            ...
            <table class="w-full text-sm">
              <thead>
                <tr class="border-b border-mp-border">
                  <th class="text-left text-xs font-semibold text-white uppercase tracking-wider px-5 py-3">Series</th>
                  <th class="text-right text-xs font-semibold text-white uppercase tracking-wider px-4 py-3">Latest</th>
                  <th class="text-right text-xs font-semibold text-white uppercase tracking-wider px-4 py-3">Min</th>
                  <th class="text-right text-xs font-semibold text-white uppercase tracking-wider px-4 py-3">Max</th>
                  <th class="text-right text-xs font-semibold text-white uppercase tracking-wider px-4 py-3">Avg</th>
                  <th class="text-right text-xs font-semibold text-white uppercase tracking-wider px-4 py-3">Total Change</th>
                  <th class="text-right text-xs font-semibold text-white uppercase tracking-wider px-4 py-3">Points</th>
                  <th class="text-center text-xs font-semibold text-white uppercase tracking-wider px-4 py-3">Detail</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-800">
                <tr v-for="stat in seriesStats" :key="stat.id" class="hover:bg-mp-card-hover/30 transition-colors">
                  <td class="px-5 py-3">
                    <div class="flex items-center gap-2">
                      ...
                      <span class="text-white text-xs">{{ stat.unit }}</span>
                    </div>
                  </td>
                  <td class="px-4 py-3 text-right font-semibold text-white tabular-nums">{{ formatValue(stat.latest) }}</td>
                  <td class="px-4 py-3 text-right text-white tabular-nums text-xs">{{ formatValue(stat.min) }}</td>
                  <td class="px-4 py-3 text-right text-white tabular-nums text-xs">{{ formatValue(stat.max) }}</td>
                  <td class="px-4 py-3 text-right text-white tabular-nums text-xs">{{ formatValue(stat.avg) }}</td>
                  <td class="px-4 py-3 text-right tabular-nums text-xs"
                    :class="stat.totalChangePct >= 0 ? 'text-mp-success' : 'text-mp-danger'">
                    {{ stat.totalChangePct !== null ? (stat.totalChangePct >= 0 ? '+' : '') + stat.totalChangePct.toFixed(2) + '%' : '—' }}
                  </td>
                  <td class="px-4 py-3 text-right text-white text-xs">{{ stat.count }}</td>
                  <td class="px-4 py-3 text-center">
                    <Link :href="`/organizations/${orgId}/statistica/${stat.id}`"
        ...
        <div v-if="allSeries.length === 0" class="bg-mp-card rounded-xl border border-dashed border-mp-border p-16 text-center">
          <p class="text-white text-sm">No series available. <Link :href="`/organizations/${orgId}/statistica`" class="text-white hover:text-white">Create some series first.</Link></p>
        </div>
...
const toggleSeries = (id) => {
  ...
  router.get(`/organizations/${props.orgId}/statistica/compare`, {
