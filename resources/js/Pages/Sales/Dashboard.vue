<template>
  <Head :title="`Sales Dashboard — ${company.name}`" />
  <AuthenticatedLayout>
    <div class="min-h-screen bg-mp-page text-mp-text-secondary">
      ...
      <div v-else class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8">
        ...
        <!-- ══════════════════════════════════════════
             SECTION 1 — SALES UPDATE KPI CARDS
        ═══════════════════════════════════════════ -->
        <div>
          <p class="text-xs font-semibold text-white uppercase tracking-widest mb-4">Sales Update</p>
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div class="bg-mp-card border border-mp-border rounded-xl p-5 relative overflow-hidden">
              ...
              <p class="text-xs text-mp-muted mt-1">Net Sales Value</p>
            ...
            <!-- Card: Avg Transaction Value -->
            <div class="bg-mp-card border border-mp-border rounded-xl p-5 relative overflow-hidden">
              <div class="absolute top-0 right-0 w-24 h-24 bg-mp-gold-dark/5 rounded-full -translate-y-6 translate-x-6"></div>
              <p class="text-xs font-semibold text-white uppercase tracking-widest mb-3">Avg Transaction Value</p>
              <p class="text-3xl font-bold text-mp-text-secondary">{{ fmt(kpis.avg_transaction) }}</p>
              <p class="text-xs text-mp-muted mt-1">Net Sales ÷ Invoice Count</p>
              <div class="mt-3">
                <span class="text-xs text-mp-muted">{{ kpis.total_invoices?.toLocaleString() }} total invoices</span>
              </div>
            </div>
            ...
            <div class="bg-mp-card border border-mp-border rounded-xl p-5 relative overflow-hidden">
              ...
              <p class="text-3xl font-bold text-mp-text-secondary">{{ kpis.active_customers?.toLocaleString() ?? '—' }}</p>
              <p class="text-xs text-mp-muted mt-1">Unique customers in period</p>
              <div class="mt-3">
                <span class="text-xs text-mp-muted">{{ kpis.total_customers?.toLocaleString() }} total ever</span>
              </div>
            </div>
          ...
          <div class="mt-4 bg-mp-card border border-mp-border rounded-xl p-6">
            <div class="flex items-center justify-between mb-4">
              <p class="text-xs font-semibold text-white uppercase tracking-widest">Monthly Net Sales Trend</p>
              <div class="flex items-center gap-4 text-xs text-mp-muted">
                <span class="flex items-center gap-1.5"><span class="w-3 h-0.5 bg-mp-teal inline-block rounded"></span> Net Sales</span>
                <span class="flex items-center gap-1.5"><span class="w-3 h-0.5 bg-mp-success inline-block rounded"></span> MoM Growth %</span>
              </div>
            </div>
        ...
        <!-- ══════════════════════════════════════════
             SECTION 2 — SALES BREAKDOWN ANALYSIS
        ═══════════════════════════════════════════ -->
        <div v-if="breakdowns.length > 0">
          <p class="text-xs font-semibold text-white uppercase tracking-widest mb-4">Sales Breakdown Analysis</p>
                ...
                <table class="w-full text-sm">
                  <thead>
                    <tr class="border-b border-mp-border">
                      <th class="text-left text-xs font-semibold text-white uppercase px-5 py-3">{{ bd.label }}</th>
                      <th class="text-right text-xs font-semibold text-white uppercase px-5 py-3">Net Sales</th>
                      <th class="text-right text-xs font-semibold text-white uppercase px-5 py-3">% Share</th>
                      <th class="text-right text-xs font-semibold text-white uppercase px-5 py-3">Accum %</th>
                    </tr>
                  </thead>
                  <tbody class="divide-y divide-gray-800">
                    <tr v-for="(row, ri) in bd.rows" :key="ri" class="hover:bg-mp-card-hover/50 transition-colors">
                      ...
                      <td class="px-5 py-2.5 text-right text-xs">
                        <span class="text-white font-semibold">{{ row.accum }}%</span>
                      </td>
                    </tr>
                  </tbody>
                  <tfoot>
                    <tr class="border-t-2 border-mp-border bg-mp-card-hover/50">
                      ...
                      <td class="px-5 py-2.5 text-right text-white font-bold text-xs">100%</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
        ...
        <!-- ══════════════════════════════════════════
             SECTION 3 — TOP ACHIEVERS
        ═══════════════════════════════════════════ -->
        <div v-if="topAchievers.length > 0">
          <p class="text-xs font-semibold text-white uppercase tracking-widest mb-4">Top Achievers</p>
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div v-if="customerNature">
          ...
          <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            <div v-for="(cat, key) in customerNature.categories" :key="key"
              ...
              <p class="text-xs font-semibold uppercase tracking-widest mb-2" :class="natureText(cat.label)">
                {{ natureLabel(cat.label) }}
              </p>
              <p class="text-3xl font-bold text-mp-text-secondary">{{ cat.count }}</p>
              <p class="text-xs mt-2 font-semibold" :class="natureText(cat.label)">
                {{ fmt(cat.total_sales) }}
              </p>
              <p class="text-xs text-mp-muted mt-1">Click for details</p>
            </div>
          </div>
          <!-- Retention metric -->
          <div v-if="customerNature" class="mt-3 bg-mp-card border border-mp-border rounded-xl px-6 py-4 flex flex-wrap gap-6">
            <div>
              <p class="text-xs text-mp-muted uppercase tracking-widest">Retention Rate</p>
              <p class="text-xl font-bold text-mp-text-secondary mt-0.5">{{ retentionRate }}%</p>
            </div>
            <div>
              <p class="text-xs text-mp-muted uppercase tracking-widest">Churn (Dead)</p>
              <p class="text-xl font-bold text-mp-danger mt-0.5">{{ customerNature.categories?.dead?.count ?? 0 }}</p>
            </div>
            <div>
              <p class="text-xs text-mp-muted uppercase tracking-widest">Reactivated</p>
              <p class="text-xl font-bold text-white mt-0.5">{{ customerNature.categories?.stop_reactivated?.count ?? 0 }}</p>
            </div>
            <div>
              <p class="text-xs text-mp-muted uppercase tracking-widest">New This Year</p>
              <p class="text-xl font-bold text-white mt-0.5">{{ customerNature.categories?.new?.count ?? 0 }}</p>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="bg-mp-card border border-mp-border rounded-xl overflow-hidden">
            ...
            <div v-if="notes.length > 0" class="divide-y divide-gray-800">
              <div v-for="(n, i) in notes" :key="n.id ?? i"
                class="p-5">
                <div class="flex items-center justify-between mb-3">
                  ...
                  <!-- Edit / Delete buttons — only show for note owner -->
                  <div class="flex items-center gap-2">
                ...
                <div class="prose-dark text-sm text-mp-text leading-relaxed"
                  v-html="n.note">
                </div>
              </div>
            </div>
            ...
            <div class="p-5">
              ...
              <div class="flex flex-wrap items-center gap-1 bg-mp-card-hover border border-mp-border rounded-t-lg px-3 py-2">
                ...
                <div class="flex items-center gap-0.5 pr-2 border-r border-mp-border">
                  <button @click="editorCmd('bold')" title="Bold"
                    :class="isActive('bold') ? 'bg-mp-teal text-mp-text-secondary' : 'text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page'"
                    class="w-7 h-7 rounded flex items-center justify-center text-sm font-bold transition-colors">B</button>
                  <button @click="editorCmd('italic')" title="Italic"
                    :class="isActive('italic') ? 'bg-mp-teal text-mp-text-secondary' : 'text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page'"
                    class="w-7 h-7 rounded flex items-center justify-center text-sm italic transition-colors">I</button>
                  <button @click="editorCmd('underline')" title="Underline"
                    :class="isActive('underline') ? 'bg-mp-teal text-mp-text-secondary' : 'text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page'"
                    class="w-7 h-7 rounded flex items-center justify-center text-sm underline transition-colors">U</button>
                  <button @click="editorCmd('strikethrough')" title="Strikethrough"
                    :class="isActive('strikethrough') ? 'bg-mp-teal text-mp-text-secondary' : 'text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page'"
                ...
                <div class="flex items-center gap-0.5 px-2 border-r border-mp-border">
                  <button @click="editorCmd('h1')" title="Heading 1"
                    :class="isActive('h1') ? 'bg-mp-teal text-mp-text-secondary' : 'text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page'"
                    class="px-2 h-7 rounded text-xs font-bold transition-colors">H1</button>
                  <button @click="editorCmd('h2')" title="Heading 2"
                    :class="isActive('h2') ? 'bg-mp-teal text-mp-text-secondary' : 'text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page'"
                    class="px-2 h-7 rounded text-xs font-bold transition-colors">H2</button>
                  <button @click="editorCmd('h3')" title="Heading 3"
                    :class="isActive('h3') ? 'bg-mp-teal text-mp-text-secondary' : 'text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page'"
                    class="px-2 h-7 rounded text-xs font-bold transition-colors">H3</button>
                </div>

                <!-- Lists -->
                <div class="flex items-center gap-0.5 px-2 border-r border-mp-border">
                  <button @click="editorCmd('bullet')" title="Bullet List"
                    :class="isActive('bullet') ? 'bg-mp-teal text-mp-text-secondary' : 'text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page'"
                    class="w-7 h-7 rounded flex items-center justify-center transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16"/></svg>
                  </button>
                  <button @click="editorCmd('ordered')" title="Numbered List"
                    :class="isActive('ordered') ? 'bg-mp-teal text-mp-text-secondary' : 'text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page'"
                    class="w-7 h-7 rounded flex items-center justify-center transition-colors">
                  ...
                  <button @click="editorCmd('blockquote')" title="Quote"
                    :class="isActive('blockquote') ? 'bg-mp-teal text-mp-text-secondary' : 'text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page'"
                    class="w-7 h-7 rounded flex items-center justify-center text-lg font-serif transition-colors">"</button>
                </div>

                <!-- Alignment -->
                <div class="flex items-center gap-0.5 px-2 border-r border-mp-border">
                  <button @click="editorCmd('alignLeft')" title="Align Left"
                    class="w-7 h-7 rounded text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page flex items-center justify-center transition-colors">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M3 5h18v2H3zm0 4h12v2H3zm0 4h18v2H3zm0 4h12v2H3z"/></svg>
                  </button>
                  <button @click="editorCmd('alignCenter')" title="Align Center"
                    class="w-7 h-7 rounded text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page flex items-center justify-center transition-colors">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M3 5h18v2H3zm3 4h12v2H6zm-3 4h18v2H3zm3 4h12v2H6z"/></svg>
                  </button>
                  <button @click="editorCmd('alignRight')" title="Align Right"
                    class="w-7 h-7 rounded text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page flex items-center justify-center transition-colors">
                ...
                <!-- Table -->
                <div class="flex items-center gap-0.5 px-2 border-r border-mp-border">
                  <button @click="editorCmd('insertTable')" title="Insert Table"
                    class="flex items-center gap-1 text-xs text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page px-2 h-7 rounded transition-colors">
                  ...
                  <button @click="editorCmd('addColBefore')" title="Add Column Before"
                    class="w-7 h-7 rounded text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page flex items-center justify-center text-xs transition-colors">C+</button>
                  <button @click="editorCmd('addRowAfter')" title="Add Row After"
                    class="w-7 h-7 rounded text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page flex items-center justify-center text-xs transition-colors">R+</button>
                  <button @click="editorCmd('deleteTable')" title="Delete Table"
                    class="w-7 h-7 rounded text-mp-danger hover:text-mp-danger hover:bg-mp-page flex items-center justify-center text-xs transition-colors">✕T</button>
                </div>

                <!-- Highlight color -->
                <div class="flex items-center gap-0.5 px-2 border-r border-mp-border">
                  <button @click="editorCmd('highlight', '#c9a84c')" title="Highlight Yellow"
                    class="w-5 h-5 rounded bg-mp-gold border border-mp-border hover:scale-110 transition-transform"></button>
                  <button @click="editorCmd('highlight', '#10b981')" title="Highlight Green"
                    class="w-5 h-5 rounded bg-mp-success border border-mp-border hover:scale-110 transition-transform"></button>
                  <button @click="editorCmd('highlight', '#ef4444')" title="Highlight Red"
                    class="w-5 h-5 rounded bg-mp-danger border border-mp-border hover:scale-110 transition-transform"></button>
                  <button @click="editorCmd('highlight', '#00b4c8')" title="Highlight Blue"
                    class="w-5 h-5 rounded bg-mp-teal border border-mp-border hover:scale-110 transition-transform"></button>
                  <button @click="editorCmd('removeHighlight')" title="Remove Highlight"
                    class="w-5 h-5 rounded bg-mp-muted border border-mp-border hover:scale-110 transition-transform text-mp-text-secondary text-xs flex items-center justify-center">✕</button>
                </div>

                <!-- Undo / Redo -->
                <div class="flex items-center gap-0.5 pl-2">
                  <button @click="editorCmd('undo')" title="Undo"
                    class="w-7 h-7 rounded text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page flex items-center justify-center transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"/></svg>
                  </button>
                  <button @click="editorCmd('redo')" title="Redo"
                    class="w-7 h-7 rounded text-mp-muted hover:text-mp-text-secondary hover:bg-mp-page flex items-center justify-center transition-colors">
              ...
              <div
                ...
                :data-placeholder="'Write your analysis, observations or action items for this period...'"
              ></div>
        ...
        <div class="bg-mp-card border border-mp-border rounded-2xl w-full max-w-2xl shadow-2xl max-h-[85vh] flex flex-col">
          ...
          <div class="p-6 overflow-y-auto flex-1">
            <div v-if="takeawayLoading" class="text-center py-8">
              <svg class="animate-spin w-8 h-8 text-white mx-auto" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/>
              </svg>
            </div>
            <div v-else-if="takeawayData">
              ...
              <div v-if="takeawaySelected" class="flex items-center gap-2 mb-4">
                <button @click="resetTakeaway"
                  class="flex items-center gap-1.5 text-xs text-white hover:text-white transition-colors">
                  <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                  </svg>
              ...
              <!-- Quick stats grid -->
              <p class="text-xs font-semibold text-white uppercase tracking-widest mb-3">
                {{ takeawaySelected ? takeawaySelected + ' — Key Stats' : 'Top ' + takeawayPopup.label + ' — Key Stats' }}
              </p>
              <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
                <div v-for="stat in takeawayData.stats" :key="stat.label"
                  class="bg-mp-card-hover rounded-xl p-4">
              ...
              <!-- Ranking list — clickable for drill-down -->
              <p class="text-xs font-semibold text-white uppercase tracking-widest mb-3">
              ...
              <table class="w-full text-sm">
                <thead>
                  <tr class="border-b border-mp-border">
                    ...
                    <th class="text-right text-xs font-semibold text-mp-muted uppercase px-3 py-2">Net Sales</th>
                ...
                <tbody class="divide-y divide-gray-800">
                  <tr v-for="(row, i) in takeawayData.ranking" :key="i"
                    ...
                    :class="[
                      takeawaySelected === row.label ? 'bg-mp-teal-subtle/40 border-l-2 border-mp-teal' : '',
                      i === 0 && !takeawaySelected ? 'bg-mp-teal-subtle/30' : '',
...
const takeawaySelected = ref(null)   // which item is drilled into
...
// SheetJS (XLSX export — loaded on demand)
...
// ── Export breakdown table to Excel ──
async function exportBreakdown(bd) {
  ...
  const dataRows = [
    [bd.label, 'Net Sales', '% Share', 'Accumulated %'],
  ...
  // Number format for Net Sales column (B2 to B last data row)
  const lastDataRow = bd.rows.length + 1
  for (let i = 2; i <= lastDataRow; i++) {
    if (ws[`B${i}`]) ws[`B${i}`].z = '#,##0.00'
  }
  ...
  // Auto-named file
  const fileName = [
    props.company.name.replace(/\s+/g, '_'),
    bd.label.replace(/\s+/g, '_'),
    dateFrom.value,
    dateTo.value
  ].join('_') + '.xlsx'
...
async function loadDashboard() {
  ...
  try {
    const { data } = await axios.get(
      route('sales.dashboard-data', props.company.id),
      { params: { date_from: dateFrom.value, date_to: dateTo.value } }
    )
    kpis.value           = data.kpis
    breakdowns.value     = (data.breakdowns || []).map(bd => ({ ...bd, tab: 'chart' }))
    topAchievers.value   = data.top_achievers || []
    customerNature.value = data.customer_nature || null
    insights.value       = data.insights || []

    // Load saved notes for this date range
    loadNotes()

    // Wait for loading=false + DOM to paint, then render charts
    loading.value = false
    await nextTick()
    await loadChartJs()
    await nextTick()
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: ctx => {
              if (ctx.datasetIndex === 0) return ' Net Sales: ' + Number(ctx.raw).toLocaleString('en-US', { maximumFractionDigits: 0 })
...
async function openTakeaway(achiever) {
  takeawayPopup.value    = achiever
  takeawaySelected.value = null
  takeawayData.value     = null
  takeawayLoading.value  = true
  await fetchTakeaway(achiever.field, null)
}

// Drill-down: user clicks a row in the ranking list
async function drillDown(itemLabel) {
  takeawaySelected.value = itemLabel
  takeawayLoading.value  = true
  await fetchTakeaway(takeawayPopup.value.field, itemLabel)
}

// Reset back to top-level (all items)
...
async function fetchTakeaway(field, selectedValue) {
  try {
    const { data } = await axios.get(
      route('sales.takeaway', props.company.id),
      {
        params: {
          field,
          selected_value: selectedValue,   // null = show top, string = drill into this value
...
// ── Nature popup ──
function openNaturePopup(cat) { naturePopup.value = cat }

// ── Computed ──
const retentionRate = computed(() => {
  if (!customerNature.value) return 0
  const cats = customerNature.value.categories || {}
  const retained = (cats.repeating?.count || 0) + (cats.active?.count || 0)
...
async function loadNotes() {
  try {
    const { data } = await axios.get(
      route('sales.get-notes', props.company.id),
      { params: { date_from: dateFrom.value, date_to: dateTo.value } }
    )
...
function startEdit(n) {
  ...
  nextTick(() => {
    if (editorEl.value) {
      editorEl.value.innerHTML = n.note
...
async function deleteNote(id) {
  ...
  try {
    await axios.delete(route('sales.delete-note', { company: props.company.id, note: id }))
    await loadNotes()
  } catch(e) { console.error(e) }
}
...
async function saveNote() {
  ...
  try {
    if (editingNoteId.value) {
      await axios.put(
        route('sales.update-note', { company: props.company.id, note: editingNoteId.value }),
        { note: html }
      )
    } else {
      await axios.post(
        route('sales.save-note', props.company.id),
...
function updateActiveFormats() {
  activeFormats.value = {
    ...
    underline: isActive('underline'), h1: isActive('h1'),
    h2: isActive('h2'), h3: isActive('h3'),
  }
}

function onEditorInput() { updateActiveFormats() }
...
function insertTable() {
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let c = 0; c < cols; c++) {
      const tag = r === 0 ? 'th' : 'td'
      const style = r === 0
        ? 'border:1px solid #1490a8;padding:8px 12px;background:#162d54;color:#00b4c8;font-weight:600;text-align:left'
  ...
  if (action === 'addCol' && table) {
    Array.from(table.rows).forEach((r, ri) => {
      ...
      newCell.style.cssText = ri === 0
        ? 'border:1px solid #1490a8;padding:8px 12px;background:#162d54;color:#00b4c8;font-weight:600'
        : 'border:1px solid #1490a833;padding:8px 12px;background:#112240;color:#e2e8f0'
      newCell.innerHTML = '&nbsp;'
      r.insertBefore(newCell, r.cells[cellIdx])
    })
  }
}
...
/* Prose styles inside rendered notes and editor */
...
.prose-dark blockquote, .editor-area blockquote {
  border-left:3px solid #00b4c8; padding-left:12px;
  color:#64748b; font-style:italic; margin:8px 0;
}
.prose-dark table, .editor-area table { border-collapse:collapse; width:100%; margin:8px 0; }
.prose-dark td, .prose-dark th,
.editor-area td, .editor-area th   { border:1px solid #1490a833; padding:8px 12px; }
.prose-dark th, .editor-area th    { background:#162d54; color:#00b4c8; font-weight:600; }
