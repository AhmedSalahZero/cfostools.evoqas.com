<template>
  <Head :title="`Upload Sales Data — ${company.name}`" />
  <AuthenticatedLayout>
    <div class="min-h-screen bg-mp-page text-white">
      ...
      <div class="bg-mp-card border-b border-mp-border">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div class="flex items-center justify-between">
            <div>
              <h1 class="text-2xl font-bold text-white">Upload Sales Data</h1>
              <p class="text-white text-sm mt-1">Upload your filled Excel template to import sales data</p>
            </div>
            <div class="hidden md:flex items-center gap-2">
              <Link :href="`/companies/${company.id}/sales/field-mapping`"
              ...
              <div class="flex items-center gap-1.5 bg-mp-teal text-white text-xs font-semibold px-3 py-1.5 rounded-full">
                <span class="w-4 h-4 rounded-full bg-white text-white flex items-center justify-center text-xs font-bold">2</span>
                Upload Data
              </div>
              <div class="w-6 h-px bg-mp-page"></div>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <!-- Info banner -->
        <div class="bg-mp-teal-subtle/30 border border-mp-teal/50 rounded-xl p-4 flex items-start gap-3">
        ...
        <div class="bg-mp-card rounded-xl border border-mp-border p-6">
          ...
          <form @submit.prevent="submit" enctype="multipart/form-data">
            ...
            <div
              ...
              @drop.prevent="onDrop"
              :class="dragOver ? 'border-mp-teal bg-mp-teal-subtle/20' : 'border-mp-border hover:border-mp-border'"
            ...
            <div class="flex items-center gap-3">
              <button type="submit" :disabled="form.processing || !selectedFile"
                class="flex items-center gap-2 bg-mp-teal hover:bg-mp-teal-dark disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium px-6 py-3 rounded-lg transition-colors text-sm">
        ...
        <div v-if="uploadList.length > 0" class="bg-mp-card rounded-xl border border-mp-border p-6">
          ...
          <div class="space-y-3">
            <div v-for="upload in uploadList" :key="upload.id"
              class="flex items-center gap-4 p-3 bg-mp-card-hover/50 rounded-lg border border-mp-border">
              ...
              <div class="flex-1 min-w-0">
                <p class="text-white text-sm font-medium">
                  {{ upload.period_from }} → {{ upload.period_to }}
                </p>
                <p class="text-white text-xs">
              ...
              <!-- Delete button -->
              <button @click="confirmDelete(upload)"
...
// Make uploads reactive so we can mutate deleting state & remove rows
...
watch(() => props.uploads, (rows) => {
  rows.forEach((row) => {
    ...
    if (existing) {
      Object.assign(existing, row, { deleting: existing.deleting })
    } else {
      uploadList.unshift({ ...row, deleting: false })
    }
  })
}, { deep: true })
...
const dateFormats = [
  { value: 'DD/MM/YYYY', label: 'Day / Month / Year',  example: '25/02/2026' },
  { value: 'MM/DD/YYYY', label: 'Month / Day / Year',  example: '02/25/2026' },
  { value: 'YYYY/MM/DD', label: 'Year / Month / Day',  example: '2026/02/25' },
  { value: 'DD-MM-YYYY', label: 'Day - Month - Year',  example: '25-02-2026' },
  { value: 'MM-DD-YYYY', label: 'Month - Day - Year',  example: '02-25-2026' },
  { value: 'YYYY-MM-DD', label: 'Year - Month - Day',  example: '2026-02-25' },
]
...
function statusClass(status) {
  ...
  return map[status] || 'bg-mp-card-hover text-white border-mp-border'
}

function submit() {
  form.post(route('sales.process-upload', props.company.id), { forceFormData: true })
}

// ── Delete flow ──
function confirmDelete(upload) {
  deleteTarget.value = upload
}
...
async function executeDelete() {
  ...
  try {
    await axios.delete(`/companies/${props.company.id}/sales/uploads/${upload.id}`)
    // Remove from list
