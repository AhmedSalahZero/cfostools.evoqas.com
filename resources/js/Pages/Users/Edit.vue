      ...
      <div class="bg-mp-card border-b border-mp-border">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div class="flex items-center justify-between">
            <div>
              <h1 class="text-2xl font-bold text-white">Edit: {{ user.name }}</h1>
              <p class="text-white text-sm mt-1">Update user details, company roles and permissions</p>
            </div>
            <div class="w-12 h-12 rounded-xl bg-mp-teal flex items-center justify-center text-xl font-bold">
        ...
        <form @submit.prevent="submit" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label class="block text-xs font-semibold text-white uppercase tracking-widest mb-2">
                  New Password
                  <span class="text-white font-normal normal-case tracking-normal ml-1">(leave blank to keep current)</span>
                </label>
                <div class="relative">
          ...
          <div class="bg-mp-card rounded-xl border border-mp-border p-6">
            ...
            <p class="text-white text-xs mb-6">
              Select companies this user can access. Assign their role and task permissions per company.
            </p>
            ...
            <div v-else class="space-y-4">
              <div v-for="company in companies" :key="company.id"
                ...
                <div class="flex items-center gap-4 p-4 cursor-pointer" @click="toggleCompany(company.id)">
                  ...
                  <!-- Show current role badge if assigned -->
                  <span v-if="isAssigned(company.id)"
                    :class="roleClass(getRole(company.id))"
                    class="text-xs px-2.5 py-1 rounded-full border font-semibold capitalize">
                    {{ getRole(company.id) }}
                  </span>
                </div>
                  ...
                  <div>
                    ...
                    <div class="grid grid-cols-3 gap-2">
                      <button v-for="role in ['manager', 'analyst', 'viewer']" :key="role"
                        ...
                        :class="getRole(company.id) === role ? roleBtnActive(role) : 'border-mp-border bg-mp-card-hover text-white hover:border-mp-border'"
...
const props = defineProps({
  ...
  assignedCompanies:  { type: Array,  default: () => [] },
})
...
const form = useForm({
  ...
  password:           '',
  assigned_companies: props.assignedCompanies.map(a => ({
    id:          a.id,
    role:        a.role,
...
function roleBtnIconBg(role) {
  const map = { manager: 'bg-mp-teal', analyst: 'bg-mp-success', viewer: 'bg-mp-muted' }
...
function roleDescription(role) {
  const map = { manager: 'Full access', analyst: 'Assigned tasks', viewer: 'View only' }
  return map[role] || ''
}
