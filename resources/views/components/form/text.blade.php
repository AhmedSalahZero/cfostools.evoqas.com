...
'smallBox'=>''
    ...
    @if(! $withoutLabel)
    <label title="{{$title}}" class=" form-label font-weight-bold   mb-2 {{$labelClass}} " for="{{$id}}">{{$label}}
        @if($required)
    <span style="color:red">*</span>
            @endif
    </label>
    @endif
    <input id="{{isset($idSuffix) && $idSuffix ? $id . $idSuffix : $id }}" title="{{$title}}" {{$attributes}} type="{{$type}}" class="form-control  mb-3 mb-lg-0 {{$inputClass}}"
