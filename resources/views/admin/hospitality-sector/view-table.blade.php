...
<div class="table-custom-container position-relative  ">
 ...
 <div class="flex-tabs mb-3">
    ...
    <x-tables.basic-view class="position-relative  main-table-class" id="{{ $tableId }}">
        <x-slot name="filter">
            @include('admin.hospitality-sector.filter' , [
            'type'=>'filter'
            ])
        </x-slot>

        <x-slot name="export">
            @include('admin.hospitality-sector.export' , [
            'type'=>'export'
            ])
        </x-slot>
...
//
                                      //              var select = `<select data-hospitality-sector-id="${row.id}" class="select select2-select change-duration-type">`;
...
//
                        ...
                        <a href="/` + $('body').data('lang') + '/' + $('body').data('current-company-id') + `/hospitality-sector/${row.id}/edit"  data-id="${row.id}" data-model-name="{{$modelName}}" class="btn btn-sm cursor-pointer btn-clean btn-icon btn-icon-md" title="{{ __('Edit Hospitality Sector Report') }}">
                        ...
                        `
                                        ...
                                        , initComplete() {
                                            ...
                                            $(document).find('.datepicker-input').datepicker({
                                                ...
                                                let hospitalitySectorId = $(this).closest('tr').data('model-id')
                                                $(this).focus().html(dateString).blur();
                                                $.ajax({
                                                    url: "{{ route('admin.update.hospitality.sector.date',getCurrentCompanyId()) }}"
                                                    , data: {
                                                        "date": dateString
        ...
        <script>
            $(function() {
                $(document).on('change', '.change-duration-type', function() {
                    ...
                    if (durationType) {
                        $.ajax({
                            url: "{{ route('admin.update.hospitality.sector.duration.type',getCurrentCompanyId()) }}"
                            , data: {
                                durationType
                                , hospitalitySectorId
                            }
