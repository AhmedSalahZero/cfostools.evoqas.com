...
@endsection
@section('sub-header')
<x-main-form-title :id="'main-form-title'" :class="''">{{ __('General & Administrative Expenses Input Sheet') }}</x-main-form-title>
        ...
        <form id="form-id" class="kt-form kt-form--label-right" method="POST" enctype="multipart/form-data" action="{{  isset($disabled) && $disabled ? '#' :  $storeRoute  }}">
			...
			@php
			  ...
			  {{-- start of Percenatge % From Rooms Revenues --}}
						@php
							$currentSectionName = variable_expenses_as_percentage_from_total_revenues;
						@endphp
                        ...
                        <table class="table table-striped table-bordered table-hover table-checkable kt_table_2 ">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">{{ __('Repeating Monthly Epenses') }}</th>
                            ...
                            <tbody>
								...
								{{-- Expense Per Night Sold --}}
									  ...
									  <td style="vertical-align:middle;text-transform:capitalize;text-align:center">
											...
											data-id="{{ $instance  }}"
											style="max-width:100px;margin-left:auto;margin-right:auto" type="text" step="0.1"  class="avg-daily-rate form-control only-greater-than-or-equal-zero-allowed" value="{{ $directExpense ? number_format($directExpense->getExpensePerNightSold()):0 }}">
									...
									<select style="width:140px;" name="opex_payment_terms[{{ $currentSectionName }}][{{ $instance }}]"  class="form-control">
                                            ...
                                            <option value="{{ $intervalArray['value'] }}"
											 @if(($directExpense ? $directExpense->getOpexPaymentTerm() : null) == $intervalArray['value'])
											selected
											@endif
