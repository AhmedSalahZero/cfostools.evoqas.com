...
@endsection
@section('sub-header')
<x-main-form-title :id="'main-form-title'" :class="''">{{ __('Rooms Department Expenses Projection Input Sheet') }}</x-main-form-title>
...
<div class="row">
    <div class="col-md-12">
									...
									<div class="col-md-10">
										<div class="d-flex align-items-center ">
											<h3 class="font-weight-bold form-label kt-subheader__title small-caps mr-5" style="">
												{{ __('Variable Expenses As % From Rooms Revenues') }}
											</h3>
										</div>
									</div>
									<div class="col-md-2">
		...
		@php
							$currentSectionName = variable_expenses_as_cost_per_night_sold;
						@endphp
   {{-- start of Variable Expenses As Cost Per Night Sold --}}
            <div class="kt-portlet">
                <div class="kt-portlet__body">
                    <div class="row">
                        <div class="col-md-10">
                            <div class="d-flex align-items-center ">
                                <h3 class="font-weight-bold form-label kt-subheader__title small-caps mr-5" style="">
                                    {{ __('Variable Expenses As Cost Per Night Sold') }}
                                </h3>
                            </div>
                        </div>
                        <div class="col-md-2">
                        ...
                        <table class="table table-striped table-bordered table-hover table-checkable kt_table_2 ">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">{{ __('Expense Per Night Sold') }}</th>
                            ...
                            <tbody>
                                ...
                                {{-- <tr>
                                    ...
                                    <td>
                                        ...
                                        <div class="form-group three-dots-parent">
                                            <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
                                                <input type="text" style="text-align: center" value="{{ number_format($totalNightsSoldCountPerYear[$year]??0 , 0) }}" readonly onchange="this.style.width = ((this.value.length + 1) * 10) + 'px';" class="form-control size trigger-change-when-start">
                                                <span class="ml-2">
                                                    <b style="visibility:hidden">%</b>
                                                </span>
                                            </div>
									...
									{{-- Night Expense At Operation Date --}}
									  <td style="vertical-align:middle;text-transform:capitalize;text-align:center">
									      ...
									      <div class="form-group three-dots-parent">
                                            <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
                                                {{-- <input type="hidden"   class="target_repeating_values  " value="0"> --}}
                                                <input readonly type="text" style="max-width: 100px;min-width: 80px;text-align: center" value="{{ number_format($directExpense ? $directExpense->getNightExpenseAtOperationDate() : 0 ) }}" data-order="{{ $order }}" data-index="{{ $instance }}"  onchange="this.style.width = ((this.value.length + 1) * 10) + 'px';" step="0.1"   data-id="{{ $instance }}" class="form-control target_repeating_amounts size html-for-adr_at_operation_date" >
												<input type="hidden" class="value-for-adr_at_operation_date" name="night_expense_at_operation_date[{{ $currentSectionName }}][{{ $instance }}]" data-id="{{ $instance}}" value="{{ $directExpense ? $directExpense->getNightExpenseAtOperationDate():0 }}" data-order="{{ $order }}" data-index="{{ $instance }}">
                                    ...
                                    <td>
                                        ...
                                        <div class="form-group three-dots-parent three-dots-column">
                                            <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
                                                {{-- <input type="hidden"   class="target_repeating_values  " value="0"> --}}
                                                <div class="position-absolute">
													<input type="text" style="max-width: 100px;min-width: 80px;text-align: center" value="{{ number_format($directExpense ? $directExpense->getNightAnnualEscalationRate() : 0 ) }}" data-order="{{ $order }}" data-index="{{ $instance }}"  onchange="this.style.width = ((this.value.length + 1) * 10) + 'px';" step="0.1"   data-id="{{ $instance }}" class="form-control target_repeating_amounts only-percentage-allowed size " >
														<input type="hidden" class="adr-escalation-rate" name="night_annual_escalation_rate[{{ $currentSectionName }}][{{ $instance }}]" data-id="{{ $instance}}" value="{{ $directExpense ? $directExpense->getNightAnnualEscalationRate():0 }}" data-order="{{ $order }}" data-index="{{ $instance }}">
													<i data-repeating-direction="column" class="custom-i-class fa fa-ellipsis-h pull-left target_last_value " data-order="{{ $order }}" data-index="{{ $instance }}" data-year="{{ $order }}" data-section="target" title="Copy Column"></i>
            ...
            {{-- end of Variable Expenses As Cost Per Night Sold --}}
                            ...
                            <tbody>
								...
								<td style="vertical-align:middle;text-transform:capitalize;text-align:left">
                                    ...
                                    <td>
                                        <div class="form-group three-dots-parent">
                                            <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
                                                <select name="chosen_guest_expense_currency[{{ $currentSectionName }}][{{ $instance }}]" data-order="{{ $order }}" class="form-control " >
                                                    @foreach($studyCurrency as $currencyId=>$currencyName)
                                                    <option value="{{ $currencyId }}"   
													@if($currencyId == ($directExpense ? $directExpense->getGuestExpenseChosenCurrency() : 0)  )
													selected
													@endif 
													>{{ $currencyName }}</option>
                                                    @endforeach
								...
								{{-- Expense Per Night Sold --}}
                                    ...
                                    @endphp			
									  <td style="vertical-align:middle;text-transform:capitalize;text-align:center">
											...
											data-calc-adr-operating-date
											data-id="{{ $instance  }}"
											style="max-width:80px;margin-left:auto;margin-right:auto;text-align:center;" type="text" step="0.1"  class="avg-daily-rate form-control only-greater-than-or-equal-zero-allowed" value="{{ $directExpense ? number_format($directExpense->getExpensePerGuestSold()):0 }}">
											<input type="hidden" name="expense_per_guest_sold[{{ $currentSectionName }}][{{ $instance }}]" class="form-control only-greater-than-or-equal-zero-allowed " value="{{ $directExpense ? $directExpense->getExpensePerGuestSold():0 }}">
										</td>
                                    ...
                                    <td>
                                        <div class="form-group three-dots-parent">
                                            <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
                                                {{-- <input type="hidden"   class="target_repeating_values  " value="0"> --}}
                                                <input   type="text" style="max-width: 100px;min-width: 80px;text-align: center" value="{{ ($model->getStudyStartDateFormattedForView()) }}" data-order="{{ $order }}"  readonly  onchange="this.style.width = ((this.value.length + 1) * 10) + 'px';"  class="form-control target_repeating_amounts  size" >
                                        ...
                                        <div class="form-group three-dots-parent">
                                            <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
												...
												<div class="position-absolute">
													                    ...
													                    <input 
												data-calc-adr-operating-date
												type="text" style="max-width: 100px;min-width: 80px;text-align: center" value="{{ number_format($directExpense ? $directExpense->getGuestExpenseEscalationRate() : 0 ) }}" data-order="{{ $order }}" data-index="{{ $instance }}"  onchange="this.style.width = ((this.value.length + 1) * 10) + 'px';" step="0.1"   data-id="{{ $instance }}" class="form-control target_repeating_amounts only-percentage-allowed size " >
												<input
												
												 type="hidden" class="adr-escalation-rate cover-value-escalation-rate" name="guest_expense_escalation_rate[{{ $currentSectionName }}][{{ $instance }}]" data-id="{{ $instance}}" value="{{ $directExpense ? $directExpense->getGuestExpenseEscalationRate():0 }}" data-order="{{ $order }}" data-index="{{ $instance }}">
											<i data-repeating-direction="column" class="custom-i-class fa fa-ellipsis-h pull-left custom-i-class target_last_value " data-order="{{ $order }}" data-index="{{ $instance }}" data-year="{{ $order }}" data-section="target" title="Copy Column"></i>
                                        ...
                                        <div class="form-group three-dots-parent">
                                            <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
                                                ...
                                                <div class="position-absolute">
													...
													<i data-repeating-direction="column" class="fa fa-ellipsis-h pull-left custom-i-class target_last_value " data-order="{{ $order }}" data-index="{{ $instance }}" data-year="{{ $order }}" data-section="target" title="Copy Column"></i>
...
<script>
    $(document).on('click', '.save-form', function(e) {
		...
		let addManpowerCount = $(this).attr('data-add-manpower-count') ? +$(this).attr('data-add-manpower-count') : 0
			...
			formData.append('has_rooms_manpower',addManpowerCount)
