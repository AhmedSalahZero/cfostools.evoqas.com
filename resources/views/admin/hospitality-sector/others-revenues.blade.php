...
@endsection
@section('sub-header')
<x-main-form-title :id="'main-form-title'" :class="''">{{ __('Other Revenues Sales Projection Input Sheet') }}</x-main-form-title>
                ...
                <div class="kt-portlet__body">
                    ...
                    <div class="table-responsive f-b-sales-project-method">
                        <table class="table table-striped table-bordered table-hover table-checkable kt_table_2">
                            <thead>
                                <tr>
                                    <th class="text-center">{{ __('Other Revenues Facility') }}</th>
                            ...
                            <tbody>
                                        ...
                                        <div class="row justify-content-center">
                                            <div class="col-md-8">
                                                <select name="f&b_facilities[all]" class="form-control blue-select all-faciltiies-select">
                                                    ...
                                                    <option  value="{{ guest_capture_x_charges_per_guest_method }}">{{ __('Guest Capture % x Charges Per Guest Method') }}</option>
                                ...
                                @foreach(count($others) ? $others : generateTotalOtherInstance($model) as $index=>$other)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="row justify-content-center">
                                            <div class="col-md-8">
                                                <select name="f&b_facilities[{{ $other->getOtherIdentifier() }}]" class="form-control blue-select facilities-per-other-select">
                                                    <option value="">{{ __('Select Method') }}</option>
                                                    <option value="{{ guest_capture_x_charges_per_guest_method }}" @if( $other->getFAndBFacilities() == guest_capture_x_charges_per_guest_method ) selected @endif>{{ __('Guest Capture % x Charges Per Guest Method') }}</option>
                                                    <option value="{{ percentage_from_rooms_revenue }}" @if( $other->getFAndBFacilities() == percentage_from_rooms_revenue ) selected @endif>{{ __('Percenatge % From Rooms Revenues') }}</option>
                                                </select>
                                            </div>
                                        </div>
                                    </td>
...
@php
				 $currentSectionName = guest_capture_x_charges_per_guest_method ;
							 @endphp
            {{-- start of Guest Capture % x Charges Per Guest Method --}}
            <div class="kt-portlet @if(isset($itemsInEachSection[$currentSectionName]))  @else d-none  @endif">
                <div class="kt-portlet__body">
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            <div class="btn active-style show-hide-repeater" data-query=".guest-capture-charges-value-per-guest-per-guest-method">{{ __('Show/Hide') }}</div>
                        </div>
                    </div>
                    <div class="row">
                        <hr style="flex:1;background-color:lightgray">
                    </div>
                    <div class="row guest-capture-charges-value-per-guest-per-guest-method">
                        ...
                        <div class="table-responsive ">
                            <table class="table table-striped table-bordered table-hover table-checkable kt_table_2">
                                <thead>
                                    <tr>
                                        <th class="text-center">{{ __('Other Revenues Facility') }}</th>
                                        <th class="text-center">{{ __('Other Revenues Facility Count') }}</th>
                                ...
                                <tbody>
                                      @foreach($itemsInEachSection[$currentSectionName]??[] as $index=>$other)
                                    <tr>
                                        ...
                                        $order = 1 ;
                                        @endphp
                                        {{-- Other Revenues Facility Count TD	 --}}
                                        <td>
                                            <div class="form-group three-dots-parent">
                                                <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
                                                    <input type="text" style="max-width: 100px;min-width: 80px;text-align: center" value="{{ $other->getOtherCount() }}" data-order="{{ $order }}" data-index="{{ $index }}" readonly onchange="this.style.width = ((this.value.length + 1) * 10) + 'px';"  class="form-control target_repeating_amounts  size">
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <div class="form-group three-dots-parent">
                                                <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
                                                    ...
                                                    <input type="text" style="max-width: 100px;min-width: 80px;text-align: center" value="{{ $other->getChargesPerGuest() }}" data-order="{{ $order }}" data-index="{{ $index }}" data-room-type-id="{{ $other->getOtherIdentifier() }}" onchange="this.style.width = ((this.value.length + 1) * 10) + 'px';"  class="form-control target_repeating_amounts  size " data-calc-adr-operating-date>
                                                    <input class="avg-daily-rate" type="hidden" name="charges_per_guest[{{ $other->getOtherIdentifier() }}]" value="{{ $other->getChargesPerGuest() ?? 0 }}" data-order="{{ $order }}" data-index="{{ $index }}" data-room-type-id="{{ $other->getOtherIdentifier() }}">
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <div class="form-group three-dots-parent">
                                                <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
                                                    <select name="chosen_other_currency[{{ $other->getOtherIdentifier() }}]" data-order="{{ $order }}" class="form-control " @if($order !=1) disabled @endif>
                                                        @foreach($studyCurrency as $currencyId=>$currencyName)
                                                        <option value="{{ $currencyId }}" @if($currencyId==( $other->getChosenCurrency()) )
                                                            selected
                                                            @endif
                                                            >{{ $currencyName }}</option>
                                                        @endforeach
                                        ...
                                        <td>
                                            <div class="form-group three-dots-parent">
                                                <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
                                                    ...
                                                    <input name="estimation_date[{{ $other->getOtherIdentifier() }}]" type="text" style="max-width: 100px;min-width: 80px;text-align: center" value="{{ $model->getStudyStartDateFormattedForView() }}" data-order="{{ $order }}" data-index="{{ $index }}" readonly onchange="this.style.width = ((this.value.length + 1) * 10) + 'px';"  class="form-control target_repeating_amounts  size">
                                        ...
                                        {{-- Charges Per Guest Escalation Rate %	 --}}
                                        ...
                                        <td>
                                            ...
                                            <div class="form-group three-dots-parent">
                                                <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
                                                    {{-- <input type="hidden"   class="target_repeating_values  " value="0"> --}}
                                                    <input type="text" style="max-width: 100px;min-width: 80px;text-align: center" value="{{ number_format($other->getChargesPerGuestEscalationRate()) }}" data-order="{{ $order }}" data-index="{{ $index }}" onchange="this.style.width = ((this.value.length + 1) * 10) + 'px';" step="0.1"  data-calc-adr-operating-date data-room-type-id="{{ $other->getOtherIdentifier() }}" class="form-control target_repeating_amounts only-percentage-allowed size ">
                                                    <input type="hidden" class="charges-value-per-guest-escalation-rate" name="charges_per_guest_escalation_rate[{{ $other->getOtherIdentifier() }}]" data-room-type-id="{{ $other->getOtherIdentifier() }}" value="{{ $other->getChargesPerGuestEscalationRate() ?? 0  }}" data-order="{{ $order }}" data-index="{{ $index }}">
                                        ...
                                        {{-- Charges Value Per Guest At Operation Date	 --}}
                                        <td>

                                            <div class="form-group three-dots-parent">
                                                <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
                                                    <input name="charges_per_guest_at_operation_date[{{ $other->getOtherIdentifier() }}]" value="{{ $other->getChargesPerGuestAtOperationDate()  }}" data-room-type-id="{{ $other->getOtherIdentifier() }}" type="hidden" class="value-for-adr_at_operation_date">
                                                    <input type="text" readonly data-room-type-id="{{ $other->getOtherIdentifier() }}" style="max-width: 100px;min-width: 80px;text-align: center" value="{{ $other->getChargesPerGuestAtOperationDate() ??0 }}" data-order="{{ $order }}" data-index="{{ $index }}" onchange="this.style.width = ((this.value.length + 1) * 10) + 'px';" step="0.1"  class="form-control target_repeating_amounts size html-for-adr_at_operation_date" data-date="#" aria-describedby="basic-addon2">
                                        ...
                                        {{-- Charges Value Per Guest Annual  Escalation Rate % --}}
                                        <td>

                                            <div class="form-group three-dots-parent">
                                                <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
                                                    {{-- average_daily_rate_annual_escalation_rate --}}
                                                    <input type="text" style="max-width: 100px;min-width: 80px;text-align: center" value="{{ number_format($other->getChargesPerGuestAnnualEscalationRate() ?? 0) }}" data-order="{{ $order }}" data-index="{{ $index }}"  onchange="this.style.width = ((this.value.length + 1) * 10) + 'px';" step="0.1"  data-room-type-id="{{ $other->getOtherIdentifier() }}" class="form-control target_repeating_amounts only-percentage-allowed size ">
                                                    <input name="charges_per_guest_annual_escalation_rate[{{ $other->getOtherIdentifier() }}]" type="hidden" value="{{ $other->getChargesPerGuestAnnualEscalationRate() ??0  }}">
                                                    <span class="ml-2">
                                                        <b>%</b>
                                                    </span>
                                                </div>
                    ...
                    <div class="table-responsive guest-capture-charges-value-per-guest-per-guest-method" >
                        <table class="table table-striped table-bordered table-hover table-checkable kt_table_2 ">
                            <thead>
                                <tr>
            ...
            {{-- end of Guest Capture % x Charges Per Guest Method --}}
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @php
                                        $currentVal =  $other->getPercentageFromRevenue($year);
                                        $currentTotal[$year]=isset($currentTotal[$year]) ? $currentTotal[$year] + $currentVal : $currentVal;
                                        @endphp
                                        <div class="form-group three-dots-parent">
                                            <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
                                                ...
                                                <input type="hidden" style="max-width: 60px;min-width: 60px;text-align: center" value="{{ $currentVal }}" data-order="{{ $order }}" data-index="{{ $index }}" name="percentage_from_rooms_revenues[{{ $other->getOtherIdentifier() }}][{{ $year }}]" onchange="this.style.width = ((this.value.length + 1) * 10) + 'px';" data-total-must-be-100="1" class="form-control target_repeating_amounts only-percentage-allowed size"  data-year="{{ $year }}" >
                                                <span class="ml-2">
                                                    <b>%</b>
                                                </span>
                                            </div>
	...
	'collectionPolicyItems'=>$others,
	'modelName'=>'other',
	'isGeneralCollectionPolicy'=>$model->isOtherGeneralCollection(),
	'isCollectionTermPerItem'=>$model->isOtherCollectionTermPerSalesChannel(),
	'onlyGeneralExpense'=>isTotal($others)
])		
...
<script>
    ...
    $(document).on('change', '.use-others', function() {
        let useOthers = $("#use-others-1").is(':checked')
        if (useOthers) {
            $('.others-repeater').fadeIn(300)
            $('input[type="radio"][name*="others"]').val(1);

        } else {
            $('.others-repeater').fadeOut(300);
            $('input[type="radio"][name*="others"]').val(0);
        }
    });
    $('.use-others').trigger('change')
...
<script>
    $(document).on('change', '[data-calc-adr-operating-date]', function() {
		...
		let ascalationRate = parent.find('.charges-value-per-guest-escalation-rate[data-room-type-id="' + roomTypeId + '"]').val() / 100;
    ...
    $(document).on('change', '.all-faciltiies-select', function() {
        let val = $(this).val()
        if (val) {
            $('.facilities-per-other-select').prop('disabled', true)
            $('.facilities-per-other-select').val(val).trigger('change')
        } else {
            $('.facilities-per-other-select').val('').trigger('change')
            $('.facilities-per-other-select').prop('disabled', false)
        }
    })
    $(function() {
