...
<x-main-form-title :id="'main-form-title'" :class="''">{{ __('Fixed Assets Statement Report') .' - '. $hospitalitySector->getStudyName() . ' - ' . $hospitalitySector->getPropertyName() }}</x-main-form-title>
    ...
    <div class="kt-portlet__body ">
        <div class="tab-content  kt-margin-t-20">
            ...
            <div class="tab-pane {{ $intervalName =='monthly' ? 'active' :'' }}" id="kt_apps_contacts_view_tab_2{{ $intervalName }}" role="tabpanel">
				...
				@foreach($reportItems as $reportFullName => $reportData)
                <x-table :tableClass="'kt_table_with_no_pagination_no_fixed removeGlobalStyle '.$intervalName">
                    @slot('table_header')
                    <tr class="first-tr-bg text-center">

                        <th style="text-transform:capitalize;white-space:nowrap">{{ $reportFullName }}</th>
                    ...
                    @endslot
                    @slot('table_body')
                    @foreach ($reportData as $reportName=>$datesAndValues)
