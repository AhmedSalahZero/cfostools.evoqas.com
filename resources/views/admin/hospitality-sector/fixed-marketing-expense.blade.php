...
<x-main-form-title :id="'main-form-title'" :class="''">{{ __('Marketing Fixed Expense Statement Report') .' - '. $hospitalitySector->getStudyName() . ' - ' . $hospitalitySector->getPropertyName() }}</x-main-form-title>
				...
				@foreach($namesIncludesTotal as $nameIncludeTotal)
                    ...
                    <x-table  :tableClass="'kt_table_with_no_pagination_no_fixed removeGlobalStyle '.$intervalName">
                            ...
                            @slot('table_body')
								...
								@foreach ($dashboardItems['prepaidExpenseStatementForMarketingForView'][$nameIncludeTotal][$intervalName]??[]   as  $reportName=>$datesAndValues)
                                    @php
