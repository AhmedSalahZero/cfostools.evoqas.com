...
<x-main-form-title :id="'main-form-title'" :class="''">{{ __('Incentive Management Fees Statement Report') .' - '. $hospitalitySector->getStudyName() . ' - ' . $hospitalitySector->getPropertyName() }}</x-main-form-title>
            ...
            <div class="tab-pane {{ $intervalName =='monthly' ? 'active' :'' }}" id="kt_apps_contacts_view_tab_2{{ $intervalName }}" role="tabpanel">
                ...
                <x-table :tableClass="'kt_table_with_no_pagination_no_fixed removeGlobalStyle '.$intervalName">
                    ...
                    <tr class="first-tr-bg text-center">
                        <th style="text-transform:capitalize;white-space:nowrap">{{ __('Item') }}</th>
                    ...
                    @endslot
                    @slot('table_body')
					@foreach ( $managementFeesStatements[$intervalName]??[] as $reportName=>$datesAndValues)

                    <tr class="first-tr" data-report-name="{{ $reportName }}">
