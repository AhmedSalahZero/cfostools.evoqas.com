...
<x-main-form-title :id="'main-form-title'" :class="''">{{ __(ucfirst($type).' Disposable Payment Statement ') .' - '. $hospitalitySector->getStudyName() . ' - ' . $hospitalitySector->getPropertyName() }}</x-main-form-title>
			...
			<div class="tab-pane {{ $intervalName =='monthly' ? 'active' :'' }}" id="kt_apps_contacts_view_tab_2{{ $intervalName }}" role="tabpanel">
				@foreach($namesIncludesTotal as $nameIncludeTotal)
                    ...
                    <x-table  :tableClass="'kt_table_with_no_pagination_no_fixed removeGlobalStyle '. $intervalName">
                            ...
                            @endslot
                            @slot('table_body')
								@foreach ($dashboardItems['disposablePaymentStatements'][$type][$nameIncludeTotal][$intervalName]??[]  as  $reportName=>$datesAndValues)
								<tr class="first-tr">
