...
@endsection
@section('sub-header')
<x-main-form-title :id="'main-form-title'" :class="''">{{ __('Property Expenses Input Sheet') }}</x-main-form-title>
...
<div class="row">
    <div class="col-md-12">
        ...
        <form id="form-id" class="kt-form kt-form--label-right" method="POST" enctype="multipart/form-data" action="{{  isset($disabled) && $disabled ? '#' :  $storeRoute  }}">
                    ...
                    <div class="table-responsive percenatge-from-rooms-revenues-method">
                        <table class="table table-striped table-bordered table-hover table-checkable kt_table_2 ">
                            ...
                            <tbody>
                                ...
                                @for($instance=0 ; $instance<5 ; $instance++) @php $directExpense=$model->getDirectExpenseForSection($modelName,$currentSectionName , $instance);
                                    @endphp
                                    <tr>
                                        <td style="vertical-align:middle;text-transform:capitalize;text-align:left">
            ...
            @php
            $currentSectionName = fixed_monthly_expenses;
            @endphp

            {{-- start of Fixed Monthly Epenses --}}
            <div class="kt-portlet">
                <div class="kt-portlet__body">
                    <div class="row">
                        <div class="col-md-10">
                            <div class="d-flex align-items-center ">
                                <h3 class="font-weight-bold form-label kt-subheader__title small-caps mr-5" style="">
                                    {{ __('Fixed Repeating Monthly Expenses') }}
                                </h3>
                            </div>
                        </div>
                        <div class="col-md-2">
                        ...
                        <table class="table table-striped table-bordered table-hover table-checkable kt_table_2 ">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">{!! __('Repeating Monthly <br> Epenses') !!}</th>
                                    ...
                                    <th class="text-center">{{ __('Payment Terms') }}</th>
                            ...
                            <tbody>
                                ...
                                @for($instance = 0 ; $instance<5 ; $instance++) @php $directExpense=$model->getDirectExpenseForSection($modelName,$currentSectionName , $instance);
                                    ...
                                    <tr>
                                            ...
                                            <div class="form-group three-dots-parent">
                                                <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
                                                    <select name="chosen_night_expense_currency[{{ $currentSectionName }}][{{ $instance }}]" data-order="{{ $order }}" class="form-control ">
                                                        ...
                                                        <option value="{{ $currencyId }}" @if($currencyId==($directExpense ? $directExpense->getNightExpenseChosenCurrency() : 0) )
                                                            selected
                                                            @endif
                                                            >{{ $currencyName }}</option>
                                                        @endforeach
                                        ...
                                        {{-- Repeating Monthly Epenses --}}


                                        <td style="vertical-align:middle;text-transform:capitalize;text-align:center">
                                            <input data-calc-adr-operating-date data-id="{{ $instance  }}" style="max-width:100px;margin-left:auto;margin-right:auto" type="text" step="0.1" class="avg-daily-rate form-control only-greater-than-or-equal-zero-allowed" value="{{ $directExpense ? number_format($directExpense->getExpensePerNightSold()):0 }}">
                                        ...
                                        <td>
                                            ...
                                            <div class="form-group three-dots-parent">
                                                <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
                                                    {{-- <input type="hidden"   class="target_repeating_values  " value="0"> --}}
                                                    <input data-calc-adr-operating-date type="text" style="max-width: 100px;min-width: 80px;text-align: center" value="{{ number_format($directExpense ? $directExpense->getNightExpenseEscalationRate() : 0 ) }}" data-order="{{ $order }}" data-index="{{ $instance }}" onchange="this.style.width = ((this.value.length + 1) * 10) + 'px';" step="0.1" data-id="{{ $instance }}" class="form-control target_repeating_amounts only-percentage-allowed size ">
                                                    <input type="hidden" class="adr-escalation-rate cover-value-escalation-rate" name="night_expense_escalation_rate[{{ $currentSectionName }}][{{ $instance }}]" data-id="{{ $instance}}" value="{{ $directExpense ? $directExpense->getNightExpenseEscalationRate():0 }}" data-order="{{ $order }}" data-index="{{ $instance }}">
                                        ...
                                        <td>
                                            ...
                                            <select style="width:140px;" name="opex_payment_terms[{{ $currentSectionName }}][{{ $instance }}]" class="form-control">
                                                @foreach(getDurationIntervalTypesForSelect() as $intervalArray)
                                                <option value="{{ $intervalArray['value'] }}" @if(($directExpense ? $directExpense->getOpexPaymentTerm() : null) == $intervalArray['value'])
                                                    selected
                                                    @endif
            ...
            @php
            $currentSectionName = other_property_fixed_expenses;
            @endphp

            {{-- start of Other Property Expenses --}}
            <div class="kt-portlet">
                <div class="kt-portlet__body">
                    <div class="row">
                        <div class="col-md-10">
                            <div class="d-flex align-items-center ">
                                <h3 class="font-weight-bold form-label kt-subheader__title small-caps mr-5" style="">
                                    {{ __('Other Property Fixed Expenses') }}
                                </h3>
                            </div>
                        </div>
                        <div class="col-md-2">
                            <div class="btn active-style show-hide-repeater" data-query=".other-property-fixed-expenses">{{ __('Show/Hide') }}</div>
                        </div>
                    </div>
                    <div class="row">
                        <hr style="flex:1;background-color:lightgray">
                    ...
                    <div class="table-responsive other-property-fixed-expenses">
                        <table class="table table-striped table-bordered table-hover table-checkable kt_table_2 ">
                            <thead>
                                <tr>
                                    <th class="text-center">{{ __('Expense Item Name') }}</th>
                                    <th class="text-center">{{ __('Choose Currency') }}</th>
                                    <th class="text-center">{!! __('Percentage From Fixed Assets') !!}</th>
                                    {{-- <th class="text-center">{{ __('Estimation Date') }}</th>
                                    <th class="text-center">{{ __('Escalation Rate %') }}</th>
                                    <th class="text-center">{{ __('Expense At Operating Date') }}</th> --}}
                                    <th class="text-center">{{ __('Annual Escalation Rate') }}</th>
                                    <th class="text-center">{{ __('Payment Month') }}</th>
                            ...
                            <tbody>
                                ...
                                @for($instance = 0 ; $instance<2 ; $instance++) @php $directExpense=$model->getDirectExpenseForSection($modelName,$currentSectionName , $instance);
                                    @endphp

                                    <tr>

                                        <td style="vertical-align:middle;text-transform:capitalize;text-align:left">
                                            <input readonly type="text" class="form-control placeholder-light-gray exclude-text" name="name[{{ $currentSectionName }}][{{ $instance }}]" 
										
											value="{{ getPropertyExpensesFroIndex($instance) }}"
											>
                                            <input type="hidden" class="form-control placeholder-light-gray" name="old_name[{{ $currentSectionName }}][{{ $instance }}]" value="{{ getPropertyExpensesFroIndex($instance) }}">
                                        </td>
                                        {{-- Choose Currency	Td --}}
                                        <td>
                                            <div class="form-group three-dots-parent">
                                        ...
                                        <td style="vertical-align:middle;text-transform:capitalize;text-align:center">
                                                          <div class="form-group three-dots-parent">
                                                <div class="input-group input-group-sm align-items-center justify-content-center div-for-percentage">
                                                    <input type="text" style="max-width: 100px;min-width: 80px;text-align: center" value="{{ number_format($directExpense ? $directExpense->getPercentageFromFixedAssets() : 0,2 ) }}" data-order="{{ $order }}" data-index="{{ $instance }}" onchange="this.style.width = ((this.value.length + 1) * 10) + 'px';" step="0.1" data-id="{{ $instance }}" class="form-control target_repeating_amounts only-percentage-allowed size ">
                                                    <input type="hidden" class="percentage-from-fixed-assets" name="percentage_from_fixed_assets[{{ $currentSectionName }}][{{ $instance }}]" data-id="{{ $instance}}" value="{{ $directExpense ? $directExpense->getPercentageFromFixedAssets():0 }}" data-order="{{ $order }}" data-index="{{ $instance }}">
                                        ...
                                        <td>
										@if($instance == 0)
                                            <select  name="payment_month[{{ $currentSectionName }}][{{ $instance }}]" class="form-control last-select-class">
                                                @foreach(getMonthsArray() as $intervalArray)
                                                <option value="{{ $intervalArray['value'] }}" @if(($directExpense ? $directExpense->getPaymentMonth() : null) == $intervalArray['value'])
                                                    selected
                                                    @endif
