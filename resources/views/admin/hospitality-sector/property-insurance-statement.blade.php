...
<x-main-form-title :id="'main-form-title'" :class="''">{{ __('Property Insurance Statement Report') .' - '. $hospitalitySector->getStudyName() . ' - ' . $hospitalitySector->getPropertyName() }}</x-main-form-title>
...
@php
            ...
            <div class="tab-pane {{ $intervalName =='monthly' ? 'active' :'' }}" id="kt_apps_contacts_view_tab_2{{ $intervalName }}" role="tabpanel">
                ...
                <x-table :tableClass="'kt_table_with_no_pagination_no_fixed removeGlobalStyle '.$intervalName">
                    ...
                    @endslot
                    @slot('table_body')
					@foreach ( $reportItems[$intervalName]??[] as $reportName=>$datesAndValues)
