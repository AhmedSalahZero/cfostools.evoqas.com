...
<div @if($isRepeater) data-repeater-item @endif class="form-group m-form__group row align-items-center 
    ...
    <div class="col-md-8">
        ...
        @else
        <x-form.select class="not-allowed-duplication-in-selection-inside-repeater" :is-required="true" :is-select2="false" :options="getOtherTypes($company,true)" :add-new="false" :label="__('Other Facility Types')" data-filter-type="{{ $type }}" :all="false" name="other_type_id" id="{{$type.'_'.'other_type_id' }}" :selected-value="isset($other) ? $other->getOtherTypeId(): 0 "></x-form.select>
        @endif
    </div>
    <div class="col-md-4">
