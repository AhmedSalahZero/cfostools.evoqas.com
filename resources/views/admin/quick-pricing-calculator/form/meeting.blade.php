				...
				<div @if($isRepeater) data-repeater-item @endif class="form-group m-form__group row align-items-center 
				    ...
				    <div class="col-md-8">
				        ...
				        @else
				        <x-form.select class="not-allowed-duplication-in-selection-inside-repeater" :is-required="true" :is-select2="false" :options="getMeetingTypes($company,true)" :add-new="false" :label="__('Meeting Facility Types')" data-filter-type="{{ $type }}" :all="false" name="meeting_type_id" id="{{$type.'_'.'meeting_type_id' }}" :selected-value="isset($meeting) ? $meeting->getMeetingTypeId(): 0 "></x-form.select>
				        @endif
				    </div>
				    <div class="col-md-2">
