 ...
 <x-tables.repeater-table :firstRowCanNotBeDeleted="false" :removeRepeater="false" :repeater-with-select2="true" :canAddNewItem="$canAddNewItem" :parentClass="'js-remove-hidden'" :hide-add-btn="true" :tableName="$tableId" :repeaterId="$repeaterId" :relationName="'meeting'" :isRepeater="$isRepeater=!(isset($removeRepeater) && $removeRepeater)">
     <x-slot name="ths">
     ...
     <x-slot name="trs">
         @php
         $rows = isset($model) ? $model->getMeetingExceptTotal() : [-1] ;
         @endphp
         @foreach( count($rows) ? $rows : [-1] as $subModel)
         @php
         if( !($subModel instanceof MeetingName) ){
         unset($subModel);
         }
         @endphp
