...
@section('content')
    <div class="row">
        <div class="col-md-12">
            ...
            <!--begin::Form-->
            <form class="kt-form kt-form--label-right" method="POST" action={{ route('uploadExcelImport', $company) }}
                ...
                <div class="kt-portlet">
                    ...
                    <div class="kt-portlet__body">
                        ...
                        <?php $active_job = App\Models\ActiveJob::where('company_id', $company->id)
                            ...
                            ->where('model_name', 'UploadExcelTest')
                            ->first(); ?>
                        ...
                        @if(! $active_job_for_saving && cache(getUploadingExcelShowCompletedTestMessageCacheKey($company->id)))
            ...
            <form action="{{ route('deleteMultiRowsFromCaching', [$company]) }}" method="POST">
                ...
                <x-table :tableTitle="__('Upload Excel Table')"
                    :href="route('uploadExcelTest.insertToMainTable',$company)" :icon="__('file-import')"
                    :firstButtonName="__('Save Data')" :tableClass="'kt_table_with_no_pagination'"
                    :truncateHref="route('deleteAllCaches',[$company,'UploadExcelTest'])"
                    ...
                    @endslot
                    @slot('table_body')
                        @foreach ($uploadExcels->take(20) as $index=> $item)
        ...
        <div class="kt-portlet text-center">
            <div class="kt-portlet__head kt-portlet__head--lg">
                <div class="kt-portlet__head-label d-flex justify-content-start">
                    {{ $uploadExcels->appends(Request::except('page'))->links() }}
                </div>
            </div>
        </div>
    </div>
            ...
            $(document).ready(function() {
                ...
                setInterval(function() {
                    ...
                    $.ajax({
                        ...
                        url: '{{ route('active.job.excel', $company) }}',
                        dataType: 'json',
        ...
        <script>
            $(document).ready(function(){
                setInterval(function(){
                     $.ajax({
                        type: 'post',
                        url: "/get-uploading-percentage-for-excel/"+{{ $company->id }},
                        data: {
                            '_token':"{{csrf_token()}}",
                        },
