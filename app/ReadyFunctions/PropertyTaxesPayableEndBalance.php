...
class PropertyTaxesPayableEndBalance  
{
	
	public function getPropertyTaxesPayableEndBalance(array $purchase , array $collection , array $dateIndexWithDate , HospitalitySector $hospitalitySector )
	{
		...
		foreach(getIntervalFormatted() as $intervalName=>$intervalNameFormatted){
			...
			foreach ($purchasesForIntervals[$intervalName]  as $dateAsIndex=>$purchaseAtDate) {
				...
				$result[$intervalName]['property_taxes'][$dateAsIndex] = $purchaseAtDate ?? 0 ; 
				$due_amount =($purchaseAtDate??0) + $beginning_balance ;
				$result[$intervalName]['due_amount'][$dateAsIndex] = $due_amount ; 
				$currenPayment =$collectionForInterval[$intervalName][$dateAsIndex]??0;
