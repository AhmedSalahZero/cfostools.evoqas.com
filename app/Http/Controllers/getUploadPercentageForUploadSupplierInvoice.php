...
class getUploadPercentageForUploadSupplierInvoice extends Controller
{
    ...
    public function __invoke(Request $request , $companyId)
    {
        ...
        $currentUploadedNumber = cache::get(getTotalUploadForUploadSupplierInvoiceCacheKey($companyId , $jobId)) ?: 0 ;
        $currentPercentage =  $totalCachedItems ? $currentUploadedNumber / $totalCachedItems * 100 : 0 ;
        $cacheHasReloadKey = Cache::has(getCanReloadUploadPageCachingForUploadSupplierInvoiceForCompany($companyId)) ;
        
        if($cacheHasReloadKey)
        {
            cache::forget(getCanReloadUploadPageCachingForUploadSupplierInvoiceForCompany($companyId));
        }
